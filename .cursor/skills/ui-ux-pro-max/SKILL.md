...
## ACC governance (this repo)

This skill is **not** Level-1/2 authority. On conflict:
`PROJECT_CONSTITUTION_V4.md` → `AGENTS.md` / `docs/ARCHITECTURE_ENFORCEMENT.md`
→ `docs/UI_CONSTITUTION.md` → this skill.

- ACC desktop UI is **CustomTkinter**, Windows-ARM64 only. Do not introduce
  React/Next/web stacks unless the user explicitly asked for a non-product
  artifact.
- Do not redesign the product UI unless asked. Do not persist a
  `design-system/` tree unless the user asked to persist.
- Search script path in this clone: `.cursor/skills/ui-ux-pro-max/scripts/search.py`
  (data dir is resolved from `__file__`, not `CLAUDE_PLUGIN_ROOT`).
...
# Step 4: stack guidelines
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "suspense streaming bundle" --stack nextjs
```
