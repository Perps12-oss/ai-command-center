...
def test_sandbox_python_c_passes_argv_allowlist_documents_risk() -> None:
    """Document: sandbox alone does not block python -c (needs control-plane gates)."""
    sandbox = CommandSandbox()
    argv = sandbox.validate_command('python -c "import os"')
    assert argv[0].lower().startswith("python")
