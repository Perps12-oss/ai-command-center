...
| P3 Event & state unification | State Authority Contract | **SOFT-SHADOW CLOSED** — Memory **ADR-015** + Goals submit **ADR-016**; workflows/executions/agents mutate still ADR-gated |
...
| State Authority | Soft-shadow Stage 2 **closed**; WM mutate WIRED; Memory `store_memory` WIRED (**ADR-015**); Goals `submit_goal` WIRED (**ADR-016**); workflows/executions/agents mutate **ADR-gated** |
