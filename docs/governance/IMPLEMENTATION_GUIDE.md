...
3. **State Authority Contract** (R1 Priority 3) — `docs/architecture/STATE_AUTHORITY_CONTRACT.md`. **Stage 2 in progress** (Slices 1–3: query, planner mandate, WM node mutate; goals dual-path / edges next).
...
2. **Stage 2 — State Authority:** **in progress** — Slices 1–3 on path (query, planner mandate, WM node mutate); shadow-SoT (goals dual-path, edges) next. No Goose unless it directly supports this work.
