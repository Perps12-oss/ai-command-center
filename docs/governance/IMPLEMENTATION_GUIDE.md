...
2. **PHASE R1 — Runtime Reconciliation** (priority order: Runtime authority → Composition → State → UI → Features; no Priority N+1 before N's gate passes). P1 **passed**. P2 registry updated; PlanningEngine/AgentCoordinator still unwired; GoalEngine **RETIRED (ADR-012 A)**. **P3 Stage 2:** Goals 3b + Memory **4a** + Workflows **5a** inventories; next Assembler 4b / Workflows 5b / executions.
3. **State Authority Contract** (R1 Priority 3) — `docs/architecture/STATE_AUTHORITY_CONTRACT.md`. **Stage 2 in progress** — soft-shadow inventories landing; deepen SA hooks only with explicit follow-ups.
...
2. **Stage 2 — State Authority:** **in progress** — Goals ADR-012 A; Memory 4a + Workflows 5a inventories on path; next Memory 4b (Assembler) or Workflows 5b (SA hooks decision). No Goose unless it directly supports this work.
