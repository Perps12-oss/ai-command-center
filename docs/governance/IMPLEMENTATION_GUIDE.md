...
| 1 | Architecture closure (Gate 1–2) | **Gate 2 CLOSED 2026-08-14** for all six streams — see [`WAVE_1_GATE_2_DECISIONS.md`](../architecture/proposals/WAVE_1_GATE_2_DECISIONS.md). A/B/C/E(read-only) ACCEPT; D/E(vectors) DEFER WITH CONDITION; F ACCEPT. **Still no code** — Gate 3 Section 9 plans are next, per stream. |
| 2–5 | Runtime foundations → capability → Goose hardening → full-system verification | Gate 3 Section 9 plans now unblocked for A, B, C, and E's read-only slice. D stays at Stage 1 measurement. F stays parked until Wave 4. |
