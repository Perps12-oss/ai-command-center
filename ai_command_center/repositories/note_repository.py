...
from ai_command_center.repositories.note_index import NoteHit, NoteIndex
...
class NoteRepository:
    ...
    def upsert(self, path: str, title: str, body: str, mtime: float) -> bool:
        return self._note_index.upsert(path, title, body, mtime)

    def remove(self, path: str) -> None:
        self._note_index.remove(path)

    def search(self, query: str, *, limit: int = 20) -> list[NoteHit]:
        return self._note_index.search(query, limit=limit)

    def get_body(self, path: str) -> str | None:
        return self._note_index.get_body(path)

    def indexed_mtime(self, path: str) -> float | None:
        return self._note_index.indexed_mtime(path)

    def count_indexed(self) -> int:
        return self._note_index.count_indexed()
