...
_TIER_DOWNGRADE_MAP: dict[str, str] = {
    "reasoning": "balanced",
    "balanced": "fast",
    "fast": "fast",
}
...
def _downgrade_routing_tier(tier: str) -> str:
    return _TIER_DOWNGRADE_MAP.get(tier, "fast")
...
class ModelRouterService(BaseService):
    ...
    def __init__(self, bus, provider_registry: ProviderRegistry | None = None) -> None:
        ...
        self._context_over_budget = False
    ...
    def _on_load(self) -> None:
        ...
        self._unsubscribers.append(
            self._bus.subscribe(CONTEXT_OVER_BUDGET, self._on_context_over_budget)
        )
        self._unsubscribers.append(
    ...
    def _on_context_over_budget(self, event: Event) -> None:
        self._context_over_budget = True
        logger.info(
            "model_router.context_over_budget tokens=%s budget=%s",
            event.payload.get("context_size_tokens"),
            event.payload.get("budget_tokens"),
        )
    ...
    @staticmethod
    def _payload_over_budget(payload: dict[str, object], sticky_flag: bool) -> bool:
        if sticky_flag or bool(payload.get("context_over_budget")):
            return True
        context_size = payload.get("context_size_tokens")
        budget = payload.get("budget_tokens")
        if context_size is None or budget is None:
            return False
        try:
            return int(context_size) >= int(budget) > 0
    ...
    def _on_resolve_request(self, event: Event) -> None:
        ...
        context_over_budget = self._payload_over_budget(event.payload, self._context_over_budget)
    ...
    def resolve(
        ...
        *,
        ...
        context_over_budget: bool = False,
    ) -> tuple[str, str]:
        ...
        if context_over_budget and routing_tier != "fast":
            routing_tier = _downgrade_routing_tier(routing_tier)
            model = self._model_for_tier(routing_tier)
