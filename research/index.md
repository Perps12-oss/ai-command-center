# ACC Engineering Intelligence Index

Master catalog for the research framework. Each sub-index owns its own domain.

## Sub-indexes

- [Repository Registry](./repositories/index.md)
- [Pattern Registry](./patterns/index.md)
- [Research Decisions](./decisions/index.md)
- [Comparisons](./comparisons/ACC_inspiration_matrix.md)
- [Integration Proposals](./integration/INT-001.md)
- [Templates](./templates/)
- [Scoring Framework](./scoring/framework.md)
- [Repository Backlog](./backlog/repositories.md)

## Lifecycle

```text
Repository Expedition
       ↓
Pattern Candidate extraction
       ↓
Pattern Validation (ACC fit, reuse, risk)
       ↓
Pattern Registry (PAT-NNN)
       ↓
Integration Proposal
       ↓
Architecture Review (Tom)
       ↓
ADR in docs/architecture/adr/
       ↓
Implementation Plan
       ↓
Implementation
       ↓
Verification
       ↓
Documentation
```

## Status overview

| Repository | Status | RD | Patterns | ADRs |
|------------|--------|----|----------|------|
| block/goose | Validated | [RD-001](./decisions/RD-001.md) | [PAT-001](./patterns/PAT-001.md), [PAT-003](./patterns/PAT-003.md), [PAT-004](./patterns/PAT-004.md), [PAT-006](./patterns/PAT-006.md), [PAT-007](./patterns/PAT-007.md) | [ADR-007](../docs/architecture/adr/ADR-007_PROVIDER_REGISTRY.md), [ADR-008](../docs/architecture/adr/ADR-008_CONVERSATION_COMPACTION.md), [ADR-009](../docs/architecture/adr/ADR-009_TOOL_CONFIRMATION_ROUTER.md), [ADR-010](../docs/architecture/adr/ADR-010_MODULAR_TOOL_INSPECTION.md), [ADR-011](../docs/architecture/adr/ADR-011_TELEMETRY_BACKENDS.md) |

*Populate as expeditions complete.*
