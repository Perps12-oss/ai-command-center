...
Task Description:
ACC Blueprint Resolutions — Phase 7: WorkflowLibrarySnapshot AppState projection.
Adds an immutable AppState.workflow_library: WorkflowLibrarySnapshot field
consolidating workflow_runs (tuple[WorkflowRunItem]) and active_workflow_run_id
into a single typed snapshot with step-level detail. Reducer consumes
WORKFLOW_STARTED, WORKFLOW_STEP_STARTED, WORKFLOW_STEP_COMPLETED,
WORKFLOW_COMPLETED, WORKFLOW_FAILED, WORKFLOW_RUNS_LOADED. All topics already in
APP_STATE_TOPICS. Existing fields preserved for backward compatibility.
No new topics, services, or DB changes.
...
- ai_command_center/core/app_state.py (lines 447-456, 515-525, 1403-1521)
...
- Existing workflow_runs, active_workflow_run_id — preserved unchanged

Sources of Truth Impacted:
- AppState source of truth: ai_command_center/core/app_state.py (new field and reducer)
- New domain module: ai_command_center/domain/workflow_library_snapshot.py
...
Contracts Impacted:
- None — all six consumed topics already registered; no new contracts required
...
workflow_runs and active_workflow_run_id unchanged.
